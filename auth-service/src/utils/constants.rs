...
fn set_token() -> SecretString {
    ...
    let secret: String = std_env::var(env::JWT_SECRET_ENV_VAR).expect("JWT_SECRET must be set.");
    if secret.is_empty() {
...
fn set_db_url() -> SecretString {
    dotenv().ok();
    SecretString::new(std_env::var(env::DATABASE_URL_ENV_VAR).expect("DATABASE_URL must be set.").into())
}
